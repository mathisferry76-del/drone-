...
// A single continuous scroll-driven "flight" that IS the hero: clouds,
// down into a house, through a bedroom, up to a gaming PC, into the
// monitor screen — which reveals a real MIN IA thumbnail. The section is
// tall (500vh of scroll runway) with the visual pinned via `sticky` while
// the user scrolls through it, the classic scrollytelling technique.
//
// Progress (0..1) lives in a ref, written directly from a scroll listener
// and read every animation frame by both the 3D scene (via useFrame, see
// JourneyScene) and the HTML overlays below (via a parallel rAF loop that
// writes to DOM style directly) — deliberately outside React state so
// scrolling never triggers a React re-render per frame.
...
export default function HeroJourney() {
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      if (flashRef.current) {
        // A hold-through trapezoid, not a spike: fully opaque for long enough
        // to completely hide the house-exterior -> bedroom cut (their fade
        // windows sit inside 0.38-0.56), not just flash past it.
        let flashOpacity = 0;
    ...
    <section ref={wrapperRef} style={{ height: `${SECTION_HEIGHT_VH}vh` }} className="relative">
      <div className="sticky top-0 h-screen w-full overflow-hidden bg-black">
        ...
        >
          ...
          <Link
            href="/generate"
            className="pointer-events-auto rounded-full bg-yellow-400 px-8 py-3 text-base font-bold text-black shadow-[0_0_40px_-8px_theme(colors.yellow.400)] transition hover:scale-105 hover:bg-yellow-300"
          >
            Créer la mienne →
          </Link>
        </div>
