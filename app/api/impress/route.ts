...
- Fais correspondre EXACTEMENT le niveau de qualité photographique réel du reste de l'image, pas une version idéalisée/nettoyée : grain du capteur, léger bruit numérique, artefacts de compression JPEG, et douceur de mise au point qu'une vraie photo de téléphone présente déjà sur le décor. L'élément modifié ne doit JAMAIS paraître plus net, plus détaillé, plus lisse ou "rendu 3D haute définition" que le reste de la photo — même si ça veut dire le rendre volontairement un peu moins net/parfait qu'un rendu par défaut. Un objet visiblement plus propre et plus défini que son environnement (comme s'il avait été détouré d'un catalogue et collé sur la photo) est un échec de compositing aussi grave qu'un mauvais angle de caméra, même si l'objet lui-même est par ailleurs fidèle au bon modèle. Cette règle porte sur le GRAIN/la NETTETÉ uniquement, jamais sur l'exposition/la luminosité : même dans une scène sombre (nuit tombante, faible lumière), l'élément modifié doit rester clairement visible et lisible — sa couleur, ses lignes de carrosserie et ses badges doivent se distinguer nettement, avec des reflets/accroches de lumière sur les surfaces brillantes qui le détachent du fond, jamais une simple silhouette sombre et indistincte plaquée sur le décor. Une vraie photo automobile professionnelle garde toujours le sujet lisible même en lumière faible ; sacrifier la lisibilité de l'objet au nom du réalisme de la scène est aussi un échec.
