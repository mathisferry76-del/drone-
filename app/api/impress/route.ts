...
IMPORTANT — une image de référence supplémentaire t'est fournie en plus de la photo à modifier : elle montre le vrai design exact de l'objet demandé (logo, motifs gravés, cadran, texte, forme précise d'un détail...). Traite-la comme une simple fiche technique de détails à extraire, JAMAIS comme une seconde scène à reproduire : n'en retiens QUE les éléments de design isolés de l'objet lui-même (forme du logo, motif, texture, agencement d'un cadran...). Elle peut montrer l'objet sous un angle totalement différent, en studio, sur un fond neutre, découpé, ou dans un tout autre contexte que la photo à modifier — ignore intégralement son propre décor, arrière-plan, sol, angle de caméra, distance, éclairage, cadrage et tout autre élément de scène qu'elle contient : rien de tout cela ne doit apparaître ou influencer le résultat. Le résultat final reproduit à 100% le décor, la composition, l'angle et l'éclairage de LA PHOTO À MODIFIER (jamais ceux de l'image de référence) ; seuls les détails de design de l'objet inséré/remplacé viennent de l'image de référence.`
    : "";
...
- Fais correspondre le NIVEAU DE QUALITÉ PHOTOGRAPHIQUE réel du reste de la photo, pas une version idéalisée : grain du capteur, léger bruit numérique, artefacts de compression, douceur de mise au point déjà présents sur le décor. L'élément modifié ne doit JAMAIS paraître plus net, plus détaillé, plus lisse ou "rendu 3D haute définition" que le reste de l'image — un objet visiblement plus propre et plus défini que son environnement (comme découpé d'un catalogue et collé sur la photo) est un échec de compositing aussi grave qu'un mauvais angle de caméra. Cette règle porte uniquement sur le GRAIN et la NETTETÉ, jamais sur l'exposition : même dans une scène sombre, l'élément modifié doit rester clairement visible et lisible (couleur, lignes de carrosserie, badges bien distincts, avec des reflets qui le détachent du fond) — sacrifier la lisibilité de l'objet au nom du réalisme du grain est aussi un échec.
