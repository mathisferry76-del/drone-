...
(b) LE CHANGEMENT DEMANDÉ N'EST PAS FAIT : soit l'ajout demandé est absent (ex : pas de montre visible si on a demandé une montre), soit — pour un REMPLACEMENT d'objet par un modèle précis (ex : "remplace ma voiture par une Ferrari") — l'objet a gardé la silhouette/forme de l'original au lieu de prendre celle du nouveau modèle demandé.
