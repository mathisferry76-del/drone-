...
D'abord, élimine toute image qui ne correspond plus à une retouche de la photo originale : scène, décor, cadrage ou sujet clairement différents de l'image 0, ou qui ne montre tout simplement pas le changement demandé. Deux façons dont un changement peut manquer, à vérifier toutes les deux : (a) l'ajout demandé est absent (ex : pas de montre visible si on a demandé une montre) ; (b) pour un REMPLACEMENT d'objet par un modèle précis (ex : "remplace ma voiture par une Ferrari"), l'objet garde la silhouette/forme de l'original au lieu de prendre celle du nouveau modèle — un objet à peine retouché qui reste visuellement le même qu'avant est un échec au même titre qu'un ajout manquant, même si l'angle et la lumière sont parfaits. Parmi les images qui restent après ce filtre, choisis celle qui a l'air la plus réaliste et physiquement intégrée à la scène, avec le logo/badge de marque (s'il y en a un) le plus net et le plus fidèle.
